-- `score / 10.0` is exact division on a numeric, so 62.00 came back as
-- "6.2000000000000000" — eighteen characters where three carry the meaning, on every film
-- of every point in the payload. IMDb states one decimal; so does this.
--
-- Only the SELECT changes; the rest is the function as the previous migration left it.
