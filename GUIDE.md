...
## 2. Словарик (чтобы не путаться)

- **Жюри / судья** (judge) — те, кто оценивает проекты и выбирает победителей. Им показываем демо.
  «Судья снимает свой рюкзак» в идеях = приём показа: даёшь приложение в руки живому человеку
  (жюри или зрителю), и он пробует его на своём. Работать всё должно и на обычном примере.
- **Демо-путь** — ОДИН сценарий, который показываем вживую от начала до конца. Его бережём как зеницу ока.
- **GitHub** — где лежит код (склад). **Vercel** — публикует код в интернете по ссылке.
  **Supabase** — хранит данные приложения. «Деплоить в GitHub» так не бывает: пушим в GitHub, публикует Vercel.
- **Ветка (branch)** — своя копия кода, где работаешь, чтобы не мешать другим. **PR (Pull Request)** —
  предложение влить свою ветку в общую. **`main`** — главная ветка, в неё сводит один человек (интегратор).
- **Скоуп** — что делаем сейчас. Всё лишнее → «вне скоупа», пока демо-путь не готов.
...
- **★ SnapSell** (Apps for your life) — фото любой вещи → готовое объявление с ценой и обоснованием. Показ: даёшь зрителю/жюри снять свою вещь — карточка за 5 секунд.
